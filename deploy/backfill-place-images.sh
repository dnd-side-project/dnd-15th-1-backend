...
readonly CONTAINER_NAME="${CONTAINER_NAME:-dulpick-place-image-backfill}"
...
if [[ $# -eq 0 ]]; then
    ...
    echo "Usage: backfill-place-images.sh [image-name image-tag]" >&2
    exit 1
fi
...
docker run --rm \
    ...
    --env SERVER_PORT=0 \
    --env PLACE_IMAGE_BACKFILL_ENABLED=true \
    --env CONTENT_THUMBNAIL_STORAGE_PATH=/var/lib/dulpick/content-images \
    "${secret_volumes[@]}" \
    "${FULL_IMAGE}"

echo "Place image backfill finished: ${FULL_IMAGE}"
