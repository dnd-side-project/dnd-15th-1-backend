...
## 7. 공용 게시물·장소 큐레이션과 데이터 공개 범위

- `contents`: 제출된 원본 게시물의 canonical URL, 제목, 내용, 썸네일, 콘텐츠 유형을 저장하는 공용 게시물이다. 기본 `publication_status`는 `PUBLIC`이다.
- `content_places`: 공용 게시물과 Kakao 검증 장소를 연결한다. 사용자가 후보를 선택하지 않아도 큐레이션을 위해 유지한다.
- `content_submissions`: 게시물을 최초 제출한 회원과 제출 시각을 기록한다. 제출자 탈퇴 후에도 공용 콘텐츠와 장소 관계는 유지한다.
- `places`: Kakao 검증을 통과한 공용 장소 정보. 모든 회원의 검색·게시물·큐레이션에서 재사용 가능하다.
- `member_places`: 실제로 저장을 선택한 회원, 별칭, 메모, 저장 시각. 본인과 현재 연결된 상대방에게만 공개한다.
- 커플 해제 후 본인 저장 정보만 유지하고 상대방 저장 정보는 차단한다.
- 새 커플에게 과거 커플의 별칭·메모를 노출하지 않는다.

게시물 원본 삭제 여부는 관리자가 일괄 확인 작업을 트리거할 수 있도록 `last_checked_at`을 둔다. 삭제가 확인된 게시물의 공개 상태 변경 정책은 운영 도구 구현 시 확정한다. 장소 자체의 삭제·폐업 처리는 게시물 원본 확인과 별도 정책으로 결정한다.

신고·차단 기능은 현재 범위에 포함하지 않는다. 관리자 기능으로 `publication_status`를 변경하는 기능은 후속 작업으로 추가한다.
...
```text
링크 입력
→ 공용 contents와 제출 이력 저장
→ 게시물 메타데이터 분석
→ 공용 places 저장 및 content_places 연결
→ 장소 후보 반환
→ 사용자가 선택
→ member_places에 개인 저장
```

사용자가 선택하지 않은 후보도 검증된 공용 게시물·장소 관계로 유지하며, 이후 공용 콘텐츠 조회 API와 큐레이션에서 활용한다.
...
- 사용자가 선택하지 않은 검증 장소도 공용 `content_places` 관계로 보존된다.
- 제출자 탈퇴 이후에도 공용 게시물과 장소 관계를 조회할 수 있다.
- 기본 공개 게시물만 공용 콘텐츠 조회 API에서 반환된다.
...
- 커플 연결 중·해제 후 장소 가시성
- 동일 canonical URL의 공용 콘텐츠 중복 방지
- 공용 게시물·장소 관계와 회원 개인 저장 관계의 분리
- 제출자 탈퇴 후 공용 콘텐츠 유지
