...
public record PushDeviceResponse(
        @Schema(description = "요청 경로로 전달한 앱 설치 단위 디바이스 UUID", requiredMode = Schema.RequiredMode.REQUIRED)
        UUID deviceId,
        @Schema(description = "푸시 발송 대상 등록 상태", allowableValues = {"ACTIVE"}, requiredMode = Schema.RequiredMode.REQUIRED)
        PushDeviceStatus status,
        @Schema(description = "FCM 토큰을 마지막으로 등록하거나 갱신한 시각", format = "date-time", requiredMode = Schema.RequiredMode.REQUIRED)
