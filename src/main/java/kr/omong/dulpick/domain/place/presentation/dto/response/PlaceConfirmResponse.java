...
public record PlaceConfirmResponse(
        @Schema(description = "후보 선택을 완료한 회원별 장소 분석 작업 ID", requiredMode = Schema.RequiredMode.REQUIRED)
        Long importId,
        @Schema(description = "후보 저장 처리 후 분석 작업 상태입니다.", requiredMode = Schema.RequiredMode.REQUIRED)
    ...
    public record SavedPlaceResponse(
            ...
            @Schema(description = "이번 확정 요청에서 새로 저장된 장소이면 true입니다.", requiredMode = Schema.RequiredMode.REQUIRED)
            boolean newlySaved
    ) {
